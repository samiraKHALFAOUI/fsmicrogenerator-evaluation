...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/supplier.repositorie");
const populate = "";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for supplier subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`supplier subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for supplier serveRPCRequest");
      break;
  }
};
