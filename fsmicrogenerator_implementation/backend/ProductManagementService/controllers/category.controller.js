...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/category.repositorie");
const populate = "parentCategory products subCategories";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for category subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`category subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for category serveRPCRequest");
      break;
  }
};
