...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/product.repositorie");
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for product subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`product subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for product serveRPCRequest");
      break;
  }
};
