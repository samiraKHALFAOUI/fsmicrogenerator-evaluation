...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/customer.repositorie");
const populate = "";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for customer subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`customer subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for customer serveRPCRequest");
      break;
  }
};
