...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/group.repositorie");
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for group subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`group subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for group serveRPCRequest");
      break;
  }
};
