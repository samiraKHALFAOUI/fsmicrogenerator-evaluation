...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/user.repositorie");
const populate = "groupe";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for user subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`user subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for user serveRPCRequest");
      break;
  }
};
