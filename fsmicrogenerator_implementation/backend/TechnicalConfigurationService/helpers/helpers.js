...
module.exports.getTranslation = (data, req) => {
  function simplifyTranslations(obj, lang, defLang) {
    ...
    for (const key in obj) {
      if (typeof obj[key] === "object" && obj[key] && Object.keys(obj[key]).length) {
...
module.exports.addData = async (req, Model, populatedFields = "") => {
  try {
    ...
    if (
      ...
      req.body.translations.language = req.headers[process.env.LANGUAGE_FIELD_NAME];
    }
...
module.exports.insertOrUpdateMany = async (req, Model, populatedFields = "") => {
  try {
    ...
    for (const doc of documents) {
      const exists = existingDocs.find(item => doc._id?.toString() === item._id.toString());
      exists ? toUpdate.push(doc) : toInsert.push(doc);
    }

    const updatePromises = toUpdate.map(doc =>
      this.updateData({ body: doc, params: { id: doc._id } }, Model, populatedFields)
    );

    const insertPromise = this.addMany({ body: toInsert }, Model, populatedFields);

    const results = await Promise.all([...updatePromises, insertPromise]);

    return results.flatMap(r =>
      Array.isArray(r) ? r :
        r?.data && r?.message ? [r.data] :
          [r]
    );

  } catch (error) {
    logger.error(`insertOrUpdateMany error (${Model.modelName}): ${error.message}`);
    throw {
      error: true,
  ...
  try {
    ...
    req.body = req.body.map(item => {
      if (
        ...
        item.translations.language = req.headers[process.env.LANGUAGE_FIELD_NAME];
      }
 ...
 */
module.exports.getData = async (req, Model, populatedFields = "", condition = {}, options = null) => {
  try {
 ...
 * @param {{ projection: import('mongoose').ProjectionType<T> ,queryOptions: import('mongoose').QueryOptions<T> }} options
 */
module.exports.getDataById = async (req, Model, populatedFields = "", options = null) => {
  try {
    const populateField = generatePopulateArray(populatedFields);
 ...
 * @param {{ projection: import('mongoose').ProjectionType<T> ,queryOptions: import('mongoose').QueryOptions<T> }} options
 */
module.exports.getOne = async (req, Model, populatedFields = "", condition = {}, options = null) => {
  try {
    const populateField = generatePopulateArray(populatedFields);
    ...
    logger.error(`getOne error (${Model.modelName}) - condition: ${JSON.stringify(condition)} - ${error.message}`);
    throw {
      error: true,
...
module.exports.getDataTranslations = async (req, Model) => {
  try {
    ...
    logger.error(`getDataTranslations error (${Model.modelName}) - id: ${req.params.id} - ${error.message}`);
    throw {
      error: true,
...
module.exports.updateData = async (req, Model, populatedFields = "") => {
  try {
    ...
    if (!req.headers || (req.body.translations && Array.isArray(req.body.translations))) {
      const result = await Model.findOneAndUpdate(
        { _id: id },
        { $set: req.body },
        { new: true, upsert: true, useFindAndModify: false }
      ).populate(populateField).lean().exec();
    ...
    const lang = req.body.translations?.language || req.headers[process.env.LANGUAGE_FIELD_NAME];
    ...
    const body = {
      ...Object.fromEntries(Object.entries({ "translations.$": _translations })),
      ...req.body
    };
    ...
    logger.error(`updateData error (${Model.modelName}) - id: ${req.params.id || req.params._id} - ${error.message}`);
    throw {
      error: true,
...
module.exports.getStatistiques = async (condition, Model) => {
  try {
      ...
      for (let key of Object.keys(item)) {
        if (typeof item[key] === "string") {
          if (item[key].includes("mongoose.Types.ObjectId")) {
            item[key] = mongoose.Types.ObjectId(item[key].match(/([0-9a-fA-F]{24})/)[1]);
          } else if (!item[key].includes("code-") && !isNaN(Date.parse(item[key]))) {
    ...
    throw {
      error: true,
      message: `Expected aggregation pipeline, got: ${JSON.stringify(condition)}`,
    };

  } catch (error) {
    logger.error(`getStatistiques error (${Model.modelName}) - ${error.message}`);
    throw {
      error: true,
    ...
    try {
      ...
      const storage = multer.diskStorage({
        ...
        filename: (req, file, cb) => {
          const name = uploadType !== "any"
            ? "1_" + file.originalname
              .replace(/[d]*_/, "")
              .replace(/-[d]*/, "") +
      ...
      switch (uploadType) {
        case "single":
          ...
          break;
        case "array":
          upload = multer({ limits, storage }).array(options.fileName, options.count || 1);
          break;
        case "fields":
      ...
      upload(req, res, (error) => {
        ...
        if (uploadType === "single" && req.file) {
          setValueToField(req.body, options.fileName, `${destination}/${req.file.filename}`);
        } else if (uploadType === "array" && req.files.length) {
          if (options.fileName.includes("$")) {
            req.files.forEach((file, index) => {
              setValueToField(req.body, options.fileName.replace("$", index), `${destination}/${file.filename}`);
            });
          } else {
            setValueToField(
              req.body,
        ...
        } else if (uploadType === "fields" && Object.keys(req.files || {}).length) {
    ...
    return arr.reduce((acc, val) =>
      acc.concat(
        (attribute ? val[attribute] : val) &&
          ...
          ? [val].concat(this.flatDeep(attribute ? val[attribute] : val, attribute))
          : val
      ),
      []
    );
...
module.exports.getDiffArray = (array1, array2, path) => {
  try {
    const a1 = Array.isArray(array1) ? array1.filter(Boolean) : [array1].filter(Boolean);
    const a2 = Array.isArray(array2) ? array2.filter(Boolean) : [array2].filter(Boolean);

    const diff = a1.filter(item1 =>
      !a2.some(item2 =>
        this.getDataByPath(item2, path)?.toString() === this.getDataByPath(item1, path)?.toString()
      )
    );
