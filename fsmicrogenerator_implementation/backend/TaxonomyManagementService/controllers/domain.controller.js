...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/domain.repositorie");
const populate = "taxonomies children";
 ...
 * @type {Handler}
 */
exports.getDomainsByCode = async (req, res) => {
  try {
    let params = JSON.parse(req.query.params || "{}");
    let domaines = JSON.parse(req.headers.domains);
    params["condition"] = { code: { $in: domaines }, etatObjet: "code-1" };
    let data = await FindAll(req, populate, params.condition, params.options);
    let sendedDoc = {};
    data.forEach((element) => {
      sendedDoc[element.code] = element;
    });
    res.status(200).send(sendedDoc);
  } catch (err) {
    return res.status(400).send(err);
  }
};
...
exports.subscribeEvents = async (payload) => {
  ...
  try {
    switch (operation) {
      ...
      default:
        logger.warn("no matching case for domain subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`domain subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for domain serveRPCRequest");
      break;
  }
};
