...
const {
  ...
  GetTranslation,
  TranslateData,
} = require("../models/repositories/taxonomy.repositorie");
const populate = "children parent";
...
exports.subscribeEvents = async (payload) => {
  ...
  try {
    switch (operation) {
      ...
      default:
        logger.warn("no matching case for taxonomy subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`taxonomy subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for taxonomy serveRPCRequest");
      break;
  }
};
