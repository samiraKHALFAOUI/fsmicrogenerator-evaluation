...
const {
  ...
  ChangeState,
  GetStatistiques,
} = require("../models/repositories/currency.repositorie");
const populate = "exchangeRates";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.info("no matching case for currency subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`currency subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.info("no matching case for currency serveRPCRequest");
      break;
  }
};
