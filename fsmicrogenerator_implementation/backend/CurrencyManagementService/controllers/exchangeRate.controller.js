...
const {
  ...
  ChangeState,
  GetStatistiques,
} = require("../models/repositories/exchangeRate.repositorie");
const populate = "currency refCurrencyBase refCurrencyEtrangere";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.info("no matching case for exchangeRate subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`exchangeRate subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.info("no matching case for exchangeRate serveRPCRequest");
      break;
  }
};
