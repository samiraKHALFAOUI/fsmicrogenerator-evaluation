...
const {
  ...
  ChangeState,
  GetStatistiques,
} = require("../models/repositories/transaction.repositorie");
const populate = "transactionLines";
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for transaction subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`transaction subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for transaction serveRPCRequest");
      break;
  }
};
