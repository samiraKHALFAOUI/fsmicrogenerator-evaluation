...
const {
  ...
  ChangeState,
  GetStatistiques,
} = require("../models/repositories/transactionLine.repositorie");
  ...
  try {
    switch (operation) {
      ...
      case "REMOVE_ITEMS":
        ...
        break;
      default:
        logger.warn("no matching case for transactionLine subscribeEvents");
        break;
    }
  } catch (error) {
    logger.error(`transactionLine subscribeEvents error: ${error}`);
  }
};
...
exports.serveRPCRequest = async (payload) => {
  ...
  switch (operation) {
    ...
    default:
      logger.warn("no matching case for transactionLine serveRPCRequest");
      break;
  }
};
