...
@Component({
  selector: "app-translate-enumeration",
  templateUrl: "./translate-enumeration.component.html",
  styleUrls: ["./translate-enumeration.component.scss"],
})
export class TranslateEnumerationComponent {
    ...
    let getData = () => {
      return this.helpers.resolve<Enumeration>((res, rej) => {
        this.enumerationService
          .getEnumerationForTranslation(this.id)
          .pipe(takeUntil(this.destroyed$))
          .subscribe({
            next: (result) => {
              res(result);
  ...
  addData() {
    ...
    if (this.form.invalid) {
      ...
      this.enumerationService
        .translateEnumeration(this.id, newForm)
        .pipe(takeUntil(this.destroyed$))
        .subscribe({
          next: (result) => {
