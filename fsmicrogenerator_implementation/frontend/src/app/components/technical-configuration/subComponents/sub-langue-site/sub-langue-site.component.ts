...
@Component({
  selector: "app-sub-langue-site",
  templateUrl: "./sub-langue-site.component.html",
  styleUrls: ["./sub-langue-site.component.scss"],
})
export class SubLangueSiteComponent {
  ...
  @Input() data: LangueSite[] = [];
  @Input() currentLangueSite!: LangueSite | undefined;
  key: string = "";
  ...
  openListDialog() {
    this.getItems(() => {
      ...
      this.ref.onClose
        .pipe(takeUntil(this.destroyed$))
        .subscribe((clonedData?: LangueSite | LangueSite[]) => {
          if (clonedData != undefined) {
            if (this.relation === "OneToMany" && clonedData.length) {
              let tempClonedData = <Partial<LangueSite>[]>clonedData;
              ...
              let tempClonedData = <LangueSite[]>clonedData;

              this.langueSiteService
                .updateManyLangueSite({
  ...
  onTranslateClick(event: any) {
    this.ref = this.dialogService.open(TranslateLangueSiteComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onAddClick() {
    this.ref = this.dialogService.open(AddLangueSiteComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onDetailClick(event: any) {
    this.ref = this.dialogService.open(DetailLangueSiteComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
