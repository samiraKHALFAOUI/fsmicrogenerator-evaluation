...
export class SubCategoryComponent {
  ...
  ngOnInit(): void {
    if (this.destination === "category" && this.relation === "OneToMany") {
    ...
    this.categoryService
      .getCategorys({
        condition: this.relation?.includes("ToOne")
          ? {}
          : { _id: { $nin: [...this.data?.map((d: any) => d._id),this.parentCategoryId] } , $or : [{parentCategory :null}, {parentCategory : {$exists :false}}]},
        options: {
          projection: {
            "translations._id": 0,
            createdAt: 0,
  ...
  openListDialog() {
    this.getItems(() => {
      ...
      this.ref.onClose
        .pipe(takeUntil(this.destroyed$))
        .subscribe((clonedData?: Category | Category[]) => {
          if (clonedData != undefined) {
            if (this.relation === "OneToMany" && clonedData.length) {
              let tempClonedData = <Partial<Category>[]>clonedData;
              if(this.destination=='category') {
                this.categoryService
                .updateManyCategory({
                  ids: tempClonedData.map((d) => d._id),
                  data: {
                    $set : {parentCategory : this.parentCategoryId}
                  },
                })
                .pipe(takeUntil(this.destroyed$))
                .subscribe({
                  next: (result) => {
                    //@ts-ignore
                    tempClonedData.map((c)=>c.parentCategory = this.parentCategoryId)
                    //@ts-ignore
                    this.data.push(...tempClonedData);
                    this.form?.setValue(this.data);
                    this.messagesService.showMessage("success");
                  },
                  error: ({ error }: any) => {
                    console.error("error", error);
                    this.messagesService.showMessage("error");
              ...
              this.categoryService
                .addManyCategory(
                  tempClonedData.map((d) => {
                    ...
                    delete d.parentCategory;
                    d[this.key] = this.parentCategoryId;
              ...
              // delete the attributes that have a oneToMany relation
              delete tempClonedData.products;
              delete tempClonedData.subCategories;
              delete tempClonedData.parentCategory;
              tempClonedData[this.key] = this.parentCategoryId;
              ...
              let tempClonedData = <Category[]>clonedData;
  ...
  onTranslateClick(event: any) {
    this.ref = this.dialogService.open(TranslateCategoryComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onDetailClick(event: any) {
    this.ref = this.dialogService.open(DetailCategoryComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
