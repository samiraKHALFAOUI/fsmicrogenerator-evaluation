...
@Component({
  ...
  styleUrls: ["./clone-product.component.scss"],
})
export class CloneProductComponent {
  pKeyFilter: RegExp = /^.*$/;
  ...
  clonedElement!: Product | undefined;
  optionsCategory :any =[]
  domains: any = ["units"];
  ...
  getProduct() {
    ...
    Promise.all([getData(), this.getDomains(), this.getAllCategories()])
      .then((result) => {
  ...
  patchValueIntoForm(data: Product) {
    ...
    delete result.inventoryMovements
    delete result.transactionLines
    result.stockQuantity = 0
    result.status = 'code_18399'
    if(this.supplierId)
