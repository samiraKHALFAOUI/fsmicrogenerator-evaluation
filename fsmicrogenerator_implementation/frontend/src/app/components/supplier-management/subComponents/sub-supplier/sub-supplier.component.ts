...
@Component({
  selector: "app-sub-supplier",
  templateUrl: "./sub-supplier.component.html",
  styleUrls: ["./sub-supplier.component.scss"],
})
export class SubSupplierComponent {
  ...
  @Input() data: Supplier[] = [];
  @Input() currentSupplier!: Supplier | undefined;
  key: string = "";
  ...
  openListDialog() {
    this.getItems(() => {
      ...
      this.ref.onClose
        .pipe(takeUntil(this.destroyed$))
        .subscribe((clonedData?: Supplier | Supplier[]) => {
          if (clonedData != undefined) {
            if (this.relation === "OneToMany" && clonedData.length) {
              let tempClonedData = <Partial<Supplier>[]>clonedData;
              ...
              let tempClonedData = <Supplier[]>clonedData;
  ...
  onTranslateClick(event: any) {
    this.ref = this.dialogService.open(TranslateSupplierComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onAddClick() {
    this.ref = this.dialogService.open(AddSupplierComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onDetailClick(event: any) {
    this.ref = this.dialogService.open(DetailSupplierComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onCloneClick(event: any) {
    this.ref = this.dialogService.open(CloneSupplierComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
