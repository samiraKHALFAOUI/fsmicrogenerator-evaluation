...
@Component({
  selector: "app-translate-domain",
  templateUrl: "./translate-domain.component.html",
  styleUrls: ["./translate-domain.component.scss"],
})
export class TranslateDomainComponent {
  ...
  getDomain() {
    let getData = () => {
      return this.helpers.resolve<Domain>((res, rej) => {
        this.domainService
          .getDomainForTranslation(this.id)
          .pipe(takeUntil(this.destroyed$))
          .subscribe({
            next: (result) => {
              res(result);
  ...
  addData() {
    ...
    if (this.form.invalid) {
      ...
      this.domainService
        .translateDomain(this.id, newForm)
        .pipe(takeUntil(this.destroyed$))
        .subscribe({
          next: (result) => {
