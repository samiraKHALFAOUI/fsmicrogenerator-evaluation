...
export class SubDomainComponent {
  ...
  @Input() data: Domain[] = [];
  @Input() currentDomain!: Domain | undefined;
  key: string = "";
  ...
  openListDialog() {
    this.getItems(() => {
      ...
      this.ref.onClose
        .pipe(takeUntil(this.destroyed$))
        .subscribe((clonedData?: Domain | Domain[]) => {
          if (clonedData != undefined) {
            if (this.relation === "OneToMany" && clonedData.length) {
              let tempClonedData = <Partial<Domain>[]>clonedData;
              ...
              // delete the attributes that have a oneToMany relation
              delete tempClonedData.taxonomies;
              delete tempClonedData.taxonomies;
              ...
              let tempClonedData = <Domain[]>clonedData;
  ...
  onTranslateClick(event: any) {
    this.ref = this.dialogService.open(TranslateDomainComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onAddClick() {
    this.ref = this.dialogService.open(AddDomainComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onDetailClick(event: any) {
    this.ref = this.dialogService.open(DetailDomainComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
