...
export class SubGroupComponent {
  ...
  @Input() data: Group[] = [];
  @Input() currentGroup!: Group | undefined;
  key: string = "";
  ...
  openListDialog() {
    this.getItems(() => {
      ...
      this.ref.onClose
        .pipe(takeUntil(this.destroyed$))
        .subscribe((clonedData?: Group | Group[]) => {
          if (clonedData != undefined) {
            if (this.relation === "OneToMany" && clonedData.length) {
              let tempClonedData = <Partial<Group>[]>clonedData;
              ...
              let tempClonedData = <Group[]>clonedData;
  ...
  onTranslateClick(event: any) {
    this.ref = this.dialogService.open(TranslateGroupComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onDetailClick(event: any) {
    this.ref = this.dialogService.open(DetailGroupComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
  ...
  onCloneClick(event: any) {
    this.ref = this.dialogService.open(CloneGroupComponent, {
      width: "90vw",
      maximizable: true,
      contentStyle: {
        "max-height": "fit-content",
