...
@Component({
  selector: "app-translate-group",
  templateUrl: "./translate-group.component.html",
  styleUrls: ["./translate-group.component.scss"],
})
export class TranslateGroupComponent {
  ...
  getGroup() {
    let getData = () => {
      return this.helpers.resolve<Group>((res, rej) => {
        this.groupService
          .getGroupForTranslation(this.id)
          .pipe(takeUntil(this.destroyed$))
          .subscribe({
            next: (result) => {
              res(result);
  ...
  addData() {
    if (this.form.invalid) {
      ...
      this.groupService
        .translateGroup(this.id, newForm)
        .pipe(takeUntil(this.destroyed$))
        .subscribe({
          next: (result) => {
